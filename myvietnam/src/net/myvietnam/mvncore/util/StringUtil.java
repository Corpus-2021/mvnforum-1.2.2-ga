/*
 * $Header: /cvsroot/mvnforum/myvietnam/src/net/myvietnam/mvncore/util/StringUtil.java,v 1.58.2.1 2008/11/19 04:44:44 lexuanttkhtn Exp $
 * $Author: lexuanttkhtn $
 * $Revision: 1.58.2.1 $
 * $Date: 2008/11/19 04:44:44 $
 *
 * ====================================================================
 *
...
public final class StringUtil {
     ...
     * All tokens are trimmed before returning
     */
...
//            } else
    ...
    public static String displayMediaContent(String url, int width, int height) {
        ...
        if (lowerURL.endsWith(".swf")) {
            result.append("<embed src='").append(url).append("' quality='high' scale='noborder' wmode='transparent'") 
                  .append(" bgcolor='#000000'").append(size).append(" type='application/x-shockwave-flash'")
                  .append(" pluginspage='http://www.macromedia.com/shockwave/download/index.cgi?P1_Prod_Version=ShockwaveFlash'></embed>");
        } else if (lowerURL.endsWith(".jpeg") || lowerURL.endsWith(".jpg") || lowerURL.endsWith(".png") || lowerURL.endsWith(".gif")) {
            result.append("<img src='").append(url).append("'").append(size).append(" border='0' alt='' />");
        } else if (lowerURL.endsWith(".mov")) {
            result.append("<object classid='clsid:02BF25D5-8C17-4B23-BC80-D3488ABDDC6B'").append(size).append(" codebase='http://www.apple.com/qtactivex/qtplugin.cab'><param name='src' value='").append(url).append("'><param name='autoplay' value='true'><param name='controller' value='false'><param name='loop' value='true'><embed src='" + url + "' width='320' height='255' autoplay='true' controller='false' loop='true' pluginspage='http://www.apple.com/quicktime/download/'></embed></object>");
        } else if (lowerURL.endsWith(".mpg") || lowerURL.endsWith(".wmv") || lowerURL.endsWith(".avi")) {
            result.append("<embed src='").append(url).append("'").append(size).append(" type='application/x-mplayer2' EnableContextMenu='0' AutoStart='1' loop='1' ShowStatusBar='0' ShowControls='0'></embed>");
        } else {
            // do nothing
        }
    ...
    public static String getReferer(HttpServletRequest request) {
        ...
        // check this again
        if ( (indexOfHost == -1) || (indexOfHost > 10) || // 10 is the maximum position when search for the host
             (referer.indexOf("/logout") != -1) || (referer.indexOf("/login") != -1) || (referer.indexOf("/register") != -1) ||
             (referer.indexOf("process") != -1) ) {
            referer = "";
        }
        return referer;
    }

    public static String getSelectionModel(int[] indexes, String[] values, String name, int selected) {
    ...
    private static void escapeLuceneSpecialCharacterString(Writer out, String str) throws IOException {
        if (str == null) {
            return;
        }
        int sz = str.length();
        for (int i = 0; i < sz; i++) {
            char ch = str.charAt(i);
            /*
             * Reference link: http://lucene.apache.org/java/docs/queryparsersyntax.html
