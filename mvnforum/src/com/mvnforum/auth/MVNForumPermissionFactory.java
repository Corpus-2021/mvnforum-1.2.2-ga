/*
 * $Header: /cvsroot/mvnforum/mvnforum/src/com/mvnforum/auth/MVNForumPermissionFactory.java,v 1.32.2.1 2008/11/19 10:09:39 thonh Exp $
 * $Author: thonh $
 * $Revision: 1.32.2.1 $
 * $Date: 2008/11/19 10:09:39 $
 *
 * ====================================================================
 *
...
public final class MVNForumPermissionFactory {
    ...
    static MVNForumPermission getAnonymousPermission()
        ...
        // we will clear 'getPermissionsForGroupGuest' cache in 2 minutes (see MemberCache.SHORT_TIME_OUT)
        ...
        // we will clear 'getPermissionsForGroupGuestInForums' cache in 2 minutes (see MemberCache.SHORT_TIME_OUT)
    ...
    public static MVNForumPermission getAuthenticatedPermission(MemberBean memberBean)
        ...
        Collection groupMemberPermList = MVNForumPermissionWebHelper.getGroupMemberPermissions(memberID);
        for (Iterator iter = groupMemberPermList.iterator(); iter.hasNext(); ) {
            int perm = ((Integer)iter.next()).intValue();
            permission.setPermission(perm);
        }

        Collection forumMemberPermList = MVNForumPermissionWebHelper.getMemberPermissionsInForums(memberID);
        for (Iterator iter = forumMemberPermList.iterator(); iter.hasNext(); ) {
            ForumPermission perm = (ForumPermission)iter.next();
            permission.setPermissionInForum(perm.getForumID(), perm.getPermission());
        }

        Collection forumGroupPermList = MVNForumPermissionWebHelper.getGroupPermissionsInForums(memberID);
        ...
        if (MvnCoreServiceFactory.getMvnCoreService().getEnvironmentService().getCmsRunMode() == EnvironmentService.PRODUCT_ENTERPRISE) {
