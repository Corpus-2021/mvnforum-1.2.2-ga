/*
 * $Header: /cvsroot/mvnforum/mvnforum/src/com/mvnforum/auth/OnlineUserManager.java,v 1.83 2008/06/12 09:25:05 minhnn Exp $
 * $Author: minhnn $
 * $Revision: 1.83 $
 * $Date: 2008/06/12 09:25:05 $
 *
 * ====================================================================
 *
...
public final class OnlineUserManager {
    ...
    public static final String LDAP_LOGIN_USERNAME = "com.mvnforum.ldap.UserName";
    ...
    private transient Collection onlineUserListeners;
    ...
    public void processLogin(HttpServletRequest request, HttpServletResponse response,
        ...
        try {
            ...
            FloodControl.ensureNotReachMaximum(MVNForumGlobal.FLOOD_ID_LOGIN_PER_IP, currentIP);

            OnlineUser user = null;
            if (memberPassword.length() > 0) {
                // that is we cannot find the md5 password
                /*
                if (MVNForumConfig.getEnableEncryptPasswordOnBrowser()) {
                    user = login(request, response, memberName, memberPassword, false);
                } else {
                    user = login(request, response, memberName, memberPassword, true);
                }*/
                user = login(request, response, memberName, memberPassword, false);
            } else {
            ...
            FloodControl.increaseCount(MVNForumGlobal.FLOOD_ID_LOGIN_PER_IP, currentIP);
            ...
            Integer maxWrongLogins = new Integer(FloodControl.getActionsPerHour(MVNForumGlobal.FLOOD_ID_LOGIN_PER_IP));
    ...
    public void logout(GenericRequest request, GenericResponse response)
        ...
        request.setSessionAttribute(LDAP_LOGIN_USERNAME, null);
        ...
        if ( (user != null) && (user.isGuest() == false) ) {
            if (user.getAuthenticationType() == OnlineUser.AUTHENTICATION_TYPE_REALM) {
                ...
                if (currentRemoteUser == null) {
                    ...
                    logout(request, null /*response*/);
                    //throw new AssertionError("ASSERTION: Current remote user should equals the current member in OnlineUser: Remote User: " + currentRemoteUser + ". But forum user is " + user.getMemberName() +". Please report bug to mvnForum developers.");
                }
        ...
        if (user.isGuest() && enableLoginInfoInRealm && (null != request.getRemoteUser())) {
            ...
            if (memberName.length() > 0) {
                try {
                    ...
                    ((OnlineUserImpl)user).setPasswordExpired(false);//external authentication never have expired password
    ...
    public OnlineUser getOnlineUser(HttpServletRequest request)
        ...
        if ( (user != null) && (user.isGuest() == false) ) {
            if (user.getAuthenticationType() == OnlineUser.AUTHENTICATION_TYPE_REALM) {
                ...
                if (currentRemoteUser == null) {
                    ...
                    logout(request, null /*response*/);
                    //throw new AssertionError("ASSERTION: Current remote user should equals the current member in OnlineUser. Please report bug to mvnForum developers.");
                }
            } else if (user.getAuthenticationType() == OnlineUser.AUTHENTICATION_TYPE_CUSTOMIZATION) {
                String currentRemoteUser = StringUtil.getEmptyStringIfNull(authenticator.getRemoteUser(request));
                if (currentRemoteUser.equals("")) {
        ...
        if (user == null) {
            ...
            if (user == null && enableLoginInfoInCAS) {
                String memberName = MvnForumServiceFactory.getMvnForumService().getCentralAuthenticationService().tryLogin(request);
    ...
    public void updateOnlineUserAction(HttpServletRequest request, Action action)
        ...
        GenericRequest genericRequest  = new GenericRequestServletImpl(request, null);
 ...
 ************************************************************************/
    ...
    protected void fireUserLogout(OnlineUserEvent e) {
        for (Iterator iterator = onlineUserListeners.iterator(); iterator.hasNext(); ) {
