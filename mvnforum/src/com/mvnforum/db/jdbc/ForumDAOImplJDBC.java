/*
 * $Header: /cvsroot/mvnforum/mvnforum/src/com/mvnforum/db/jdbc/ForumDAOImplJDBC.java,v 1.44 2008/06/06 10:46:45 minhnn Exp $
 * $Author: minhnn $
 * $Revision: 1.44 $
 * $Date: 2008/06/06 10:46:45 $
 *
 * ====================================================================
 *
...
public class ForumDAOImplJDBC implements ForumDAO {
    ...
    private static final Log log = LogFactory.getLog(ForumDAOImplJDBC.class);
     ...
     *
     */
    public Collection getBeans_withSortSupport_limit_mysql(int offset, int rowsToReturn, String sort, String order) throws IllegalArgumentException, DatabaseException {
        if (offset < 0) throw new IllegalArgumentException("The offset < 0 is not allowed.");
        if (rowsToReturn <= 0) throw new IllegalArgumentException("The rowsToReturn <= 0 is not allowed.");
     ...
     *
     */
    public Collection getBeans_withSortSupport_limit_noscroll(int offset, int rowsToReturn, String sort, String order) throws IllegalArgumentException, DatabaseException {
        if (offset < 0) throw new IllegalArgumentException("The offset < 0 is not allowed.");
        if (rowsToReturn <= 0) throw new IllegalArgumentException("The rowsToReturn <= 0 is not allowed.");
    ...
    public Collection getForums_withSortSupport_limit_ViewCount(int offset, int rowsToReturn, String sort, String order) 
        ...
        if (DBUtils.getDatabaseType() == DBUtils.DATABASE_ORACLE) {
            sql.append("SELECT mvnforumForum.ForumID, CategoryID, ForumOwnerName, mvnforumForum.LastPostMemberName, ForumName, ForumDesc, ForumCreationDate, ForumModifiedDate, ForumLastPostDate, ForumOrder, ForumType, ForumFormatOption, ForumOption, ForumStatus, ForumModerationMode, ForumPassword, ForumThreadCount, ForumPostCount, ViewCount");
            sql.append(" FROM (");
            sql.append("       SELECT mvnforumForum.ForumID AS id, SUM(mvnforumThread.ThreadViewCount) AS ViewCount");
            sql.append("       From mvnforumForum, mvnforumThread");
            sql.append("       WHERE mvnforumForum.ForumID = mvnforumThread.ForumID (+)");
            sql.append("       GROUP BY mvnforumForum.ForumID), mvnforumForum");
            sql.append(" WHERE mvnforumForum.ForumID = id");
            sql.append(" ORDER BY " + sort + " " + order);
        } else {
            sql.append("SELECT DISTINCT mvnforumForum.ForumID, CategoryID, ForumOwnerName, mvnforumForum.LastPostMemberName, ForumName, ForumDesc,ForumCreationDate, ForumModifiedDate, ForumLastPostDate, ForumOrder, ForumType, ForumFormatOption, ForumOption, ForumStatus, ForumModerationMode, ForumPassword, ForumThreadCount, ForumPostCount, SUM(mvnforumThread.ThreadViewCount) AS ViewCount");
            sql.append(" FROM mvnforumForum LEFT OUTER JOIN mvnforumThread ON mvnforumForum.ForumID=mvnforumThread.ForumID");
        ...
        try {
            ...
            log.error("Sql Execution Error!", sqle);
            throw new DatabaseException("Error executing SQL in ForumDAOImplJDBC.getBeans_withSortSupport_limit_ViewCount_general.");
        } finally {
            DBUtils.closeResultSet(resultSet);
            DBUtils.resetStatement(statement);
            DBUtils.closeStatement(statement);
