/*
 * $Header: /cvsroot/mvnforum/mvnforum/src/com/mvnforum/db/jdbc/MemberGroupDAOImplJDBC.java,v 1.22 2008/05/30 04:41:31 minhnn Exp $
 * $Author: minhnn $
 * $Revision: 1.22 $
...
public class MemberGroupDAOImplJDBC implements MemberGroupDAO {
    ...
    private static final Log log = LogFactory.getLog(MemberGroupDAOImplJDBC.class);
 ...
 ************************************************/
    ...
    public Collection getBeans_limit(int groupID, int offset, int rowsToReturn)
        ...
        Collection retValue = new ArrayList();
        StringBuffer sql = new StringBuffer(512);
        sql.append("SELECT GroupID, MemberID, MemberName, Privilege, CreationDate, ModifiedDate");
        sql.append(" FROM " + TABLE_NAME);
        sql.append(" WHERE GroupID = ?");
        sql.append(" LIMIT ?, ?");
        try {
