/*
 * $Header: /cvsroot/mvnforum/mvnforum/src/com/mvnforum/search/attachment/AttachmentIndexer.java,v 1.10 2008/05/30 04:41:34 minhnn Exp $
 * $Author: minhnn $
 * $Revision: 1.10 $
...
public class AttachmentIndexer {
    ...
    private static final Log log = LogFactory.getLog(AttachmentIndexer.class);
    ...
    static void doIndexAttachment(AttachmentBean attachmentBean, IndexWriter writer) throws SearchException {
        ...
        //Each Atachment will be represented as a document
        Document attachmentDocument = new Document();
        //Document has following fields that could be queried on
        attachmentDocument.add(new Field(FIELD_ATTACHMENT_ID, Integer.toString(attachmentBean.getAttachID()), Field.Store.YES, Field.Index.UN_TOKENIZED));
        attachmentDocument.add(new Field(FIELD_ATTACHMENT_NAME, attachmentBean.getAttachFilename(), Field.Store.YES, Field.Index.TOKENIZED));
        attachmentDocument.add(new Field(FIELD_ATTACHMENT_DESCRIPTION, attachmentBean.getAttachDesc(), Field.Store.YES, Field.Index.TOKENIZED));
        attachmentDocument.add(new Field(FIELD_POST_ID, Integer.toString(attachmentBean.getPostID()), Field.Store.YES, Field.Index.UN_TOKENIZED));
        attachmentDocument.add(new Field(FIELD_FORUM_ID, Integer.toString(attachmentBean.getForumID()), Field.Store.YES, Field.Index.UN_TOKENIZED));
        attachmentDocument.add(new Field(FIELD_ATTACHMENT_DATE, DateTools.dateToString(attachmentBean.getAttachCreationDate(), Resolution.MILLISECOND), Field.Store.YES, Field.Index.UN_TOKENIZED));
