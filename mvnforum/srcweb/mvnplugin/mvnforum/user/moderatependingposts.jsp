<%--
 - $Header: /cvsroot/mvnforum/mvnforum/srcweb/mvnplugin/mvnforum/user/moderatependingposts.jsp,v 1.75 2008/06/13 09:23:21 phuongpdd Exp $
 - $Author: phuongpdd $
 - $Revision: 1.75 $
...
<table width="95%" align="center">
  <tr class="nav">
    ...
    <a class="nav" href="<%=urlResolver.encodeURL(request, response, "listthreadswithpendingposts?forum=" + forumID)%>"><fmt:message key="mvnforum.user.listthreadswithpendingposts.title"/></a>&nbsp;&raquo;&nbsp;
        ...
        <tr>
          <td width="100%" valign="top" class="messageTextBold">
            <% if (postBean.getParentPostID() != 0) { // replied post %>
              <a href="<%=urlResolver.encodeURL(request, response, "editpost?post=" + postBean.getPostID() + "&amp;offset=0")%>"><img src="<%=contextPath%>/mvnplugin/mvnforum/images/icon/button_edit.gif" border="0" alt="<fmt:message key="mvnforum.user.viewthread.link.edit_post"/>" title="<fmt:message key="mvnforum.user.viewthread.link.edit_post"/>" /></a>&nbsp;&nbsp;
            <% } %>
