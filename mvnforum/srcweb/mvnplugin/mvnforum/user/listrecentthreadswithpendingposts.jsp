<%--
 - $Header: /cvsroot/mvnforum/mvnforum/srcweb/mvnplugin/mvnforum/user/listrecentthreadswithpendingposts.jsp,v 1.68 2008/06/13 09:23:21 phuongpdd Exp $
 - $Author: phuongpdd $
 - $Revision: 1.68 $
...
<mvn:head>
  <mvn:title><fmt:message key="mvnforum.common.forum.title_name"/> - <fmt:message key="mvnforum.user.listrecentthreadswithpendingposts.title"/></mvn:title>
...
<table width="95%" align="center">
  <tr class="nav">
    ...
    <a class="nav" href="<%=urlResolver.encodeURL(request, response, "modcp")%>"><fmt:message key="mvnforum.user.modcp.title"/></a>&nbsp;&raquo;&nbsp;
    <fmt:message key="mvnforum.user.listrecentthreadswithpendingposts.title"/>
    </td>
  </tr>
</table>
<br/>
...
<table width="95%" align="center">
  <tr>
    ...
    <form name="form" action="<%=urlResolver.encodeURL(request, response, "listrecentthreadswithpendingposts", URLResolverService.ACTION_URL)%>" <mvn:method/>>
      <%=urlResolver.generateFormAction(request, response, "listrecentthreadswithpendingposts")%>
...
<pg:pager
  url="listrecentthreadswithpendingposts"
  items="<%= totalThreads %>"
...
<pg:item>
  <tr class="<mvn:cssrow/>">
    ...
    <td>
      <a href="<%=urlResolver.encodeURL(request, response, "listthreadswithpendingposts?forum=" + threadBean.getForumID())%>" class="messageTopic"><%=forumCache.getBean(threadBean.getForumID()).getForumName()%></a>
    </td>
    <td align="center">
...
if (threadBeans.size() == 0) {%>
  <tr class="<mvn:cssrow/>"><td colspan="8" align="center"><fmt:message key="mvnforum.user.listrecentthreadswithpendingposts.table.no_threads"/></td></tr>
<%}%>
</mvn:cssrows>
</table>
