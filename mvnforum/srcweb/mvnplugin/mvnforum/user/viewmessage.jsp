<%--
 - $Header: /cvsroot/mvnforum/mvnforum/srcweb/mvnplugin/mvnforum/user/viewmessage.jsp,v 1.89 2008/06/13 09:23:22 phuongpdd Exp $
 - $Author: phuongpdd $
 - $Revision: 1.89 $
...
<table width="95%" align="center">
  <tr class="nav">
    ...
    <fmt:message key="mvnforum.common.private_message.folder"/>: <a class="nav" href="<%=urlResolver.encodeURL(request, response, "mymessage?folder=" + messageBean.getFolderName())%>"><%=messageBean.getFolderName()%></a>&nbsp;&raquo;&nbsp;
  ...
  <tr class="portlet-font"><td>
    <form action="">
      ...
      <select name="FastGoFolder" onchange="gotoPage(this.options[this.selectedIndex].value)">
        ...
        <option value="<%=urlResolver.encodeURL(request, response, "mymessage?folder=Inbox")%>">Inbox</option>
        <option value="<%=urlResolver.encodeURL(request, response, "mymessage?folder=Sent")%>">Sent</option>
        <option value="<%=urlResolver.encodeURL(request, response, "mymessage?folder=Draft")%>">Draft</option>
      </select>
    </form>
  </td></tr>
</table>
...
<table class="tborder" width="95%" cellspacing="0" cellpadding="3" align="center">
  <tr class="pagedesc">
    <td align="center" class="warning"><fmt:message key="mvnforum.common.private_message.type.quote_message"/></td>
  </tr>
</table>
<br/>
<%}%>
    ...
    <td>
      <table width="100%" class="noborder" cellpadding="0" cellspacing="0">
        <tr>
          ...
          <td width="100%">
          <%if (memberSenderBean != null) {%>
            &nbsp;&nbsp;&nbsp;<a href="<%=urlResolver.encodeURL(request, response, "viewmember?member=" + Encoder.encodeURL(memberSenderBean.getMemberName()))%>"><img src="<%=contextPath%>/mvnplugin/mvnforum/images/icon/button_profile.gif" border="0" alt="" /></a>
            <%if (memberSenderBean.getMemberEmailVisible() == 1) {%>
              &nbsp;&nbsp;&nbsp;<a href="mailto:<%=memberSenderBean.getMemberEmail()%>"><img src="<%=contextPath%>/mvnplugin/mvnforum/images/icon/button_email.gif" border="0" alt="<%=memberSenderBean.getMemberEmail()%>" title="<%=memberSenderBean.getMemberEmail()%>" /></a>
            <%}%>
